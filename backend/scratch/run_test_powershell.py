...
print("=== Starting Test Uvicorn Server via Shell Redirection ===")
if os.path.exists(log_file_path):
    os.remove(log_file_path)

cmd = (
    'C:\\Users\\Varshini\\OneDrive\\Desktop\\MeetingPilot-AI\\.venv\\Scripts\\python.exe -m uvicorn app.main:create_app '
    '--host 127.0.0.1 --port 8001 --factory > scratch/uvicorn_test.log 2>&1'
)
...
try:
    ...
    # On Windows, terminating a shell process requires killing the process tree
    subprocess.run("taskkill /F /T /PID " + str(proc.pid), shell=True)
    print("Server terminated.")

# Print logs
print("\n=== Uvicorn Server Logs ===")
time.sleep(1) # wait for OS file handles to flush
if os.path.exists(log_file_path):
